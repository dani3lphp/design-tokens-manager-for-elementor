...
/**
 * Activation hook: ensure plugin options exist.
 *
 * @return void
 */
 ...
 *
 * @param string $unused_hook_suffix Current admin page hook (unused).
 *
 * @phpstan-param string $unused_hook_suffix
 * @psalm-param string $unused_hook_suffix
 * @return void
 */
function edtm_enqueue_admin_assets( $unused_hook_suffix ) {
	// Mark parameter as intentionally unused to satisfy sniffs.
	unset( $unused_hook_suffix );
	...
	// Add inline CSS to control initial panel visibility without inline <style> in markup.
	$__edtm_section = 'fonts';
	// phpcs:disable WordPress.Security.NonceVerification
	if ( isset( $_GET['edtm_section'] ) ) {
		$__edtm_raw = sanitize_text_field( wp_unslash( $_GET['edtm_section'] ) );
		if ( in_array( $__edtm_raw, array( 'colors', 'fonts' ), true ) ) {
			$__edtm_section = $__edtm_raw;
		}
	} elseif ( isset( $_POST['edtm_current_section'] ) ) { // Read-only UI state; no data written.

		$__edtm_raw = sanitize_text_field( wp_unslash( $_POST['edtm_current_section'] ) );
		if ( in_array( $__edtm_raw, array( 'colors', 'fonts' ), true ) ) {
			$__edtm_section = $__edtm_raw;
		}
	} else {
	...
	// phpcs:enable
	$__edtm_css = ".edtm-panel[data-section]{display:none;}\n";
	if ( 'colors' === $__edtm_section ) {
		$__edtm_css .= ".edtm-panel[data-section=\\\"colors\\\"]{display:block;}\n";
	} else {
		$__edtm_css .= ".edtm-panel[data-section=\\\"fonts\\\"]{display:block;}\n";
	}
	wp_add_inline_style( 'edtm-admin', $__edtm_css );
...
function edtm_print_sync_notice() {
	...
	if ( ! empty( $notice['message'] ) ) {
		...
		if ( 'warning' === $type ) {
			$class = 'notice-warning'; } elseif ( 'error' === $type ) {
			$class = 'notice-error'; }
...
// WP-CLI (optional).
if ( defined( 'WP_CLI' ) && WP_CLI && file_exists( EDTM_PATH . 'includes/class-edtm-cli-command.php' ) ) {
	require_once EDTM_PATH . 'includes/class-edtm-cli-command.php';
}
